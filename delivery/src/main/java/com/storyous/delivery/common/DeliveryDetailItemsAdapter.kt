...
class DeliveryDetailItemsAdapter :
    ...
    class ItemDetailViewHolder(
        ...
        fun bind(item: DeliveryItem) {
            count.text = Configuration.formatter.formatCount(item.count)
            measure.text = item.measure ?: getString(R.string.piece_measure)
            title.text = item.title

            price.text = Configuration.formatter.formatPriceWithoutCurrency(
                item.unitPriceWithVat,
                item.count
            )
            currency.text = Configuration.formatter.defaultCurrency(itemView.context)
        ...
        private fun bindSubItem(
            ...
            val formattedPriceWithoutCurrency = Configuration.formatter.formatPriceWithoutCurrency(
            ...
            subItemView.price_currency.text =
                Configuration.formatter.defaultCurrency(subItemView.context)
        }
    }
}
